...
無線LANローミングシステムを構築するには，実際に利用するためにはアクセスポイント(AP)や認証サーバ(認証方式にIEEE802.1Xを用いる場合)のコンフィグレーションを行う必要があり，また，APにつなぐLANケーブルや電源ケーブルを配線するのに，時間や労力がかかります．一方，災害時などの緊急時に即座に(安全な認証方式に基づく)無線ネットワークを構築することができれば，情報通信の手段を素早く確保することができると考えられます．私はそのような簡単にデプロイ可能であるかつ，安全な認証方式を有する無線LANシステムの実現を目指して，無線LANの認証プロトコルや無線メッシュネットワーク技術についての研究開発を行っております．
